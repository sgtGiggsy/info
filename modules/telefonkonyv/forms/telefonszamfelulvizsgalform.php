...
if(@$irhat)
{
    ?><form action="<?=$RootPath?>/telefonkonyv/felulvizsgalat&action=review" method="post" onsubmit="beKuld.disabled = true; return true;"><?php
        ...
        ?><div class="ketharmad" >
            ...
            if(!$valtozas['torolve'])
            {
                ?><div>
                    ...
                    <div>
                        ...
                        <textarea name="megjegyzes" id="megjegyzes" <?=($megjegyzes != $origmegjegyzes) ? "style='background-color: yellow; color: black'; title='" . $origmegjegyzes . "'" : "" ?> onchange="setAllapotPartial()"><?=$megjegyzes?></textarea>
                    </div>
            ...
            {
                ?><input type ="hidden" id="torles" name="torles" value="1">
                <input type ="hidden" id="csoport" name="csoport" value="<?=$csoportid?>"><?php
            }
            ...
            ?><div><?php
                ...
                <?php $magyarazat .= "<strong>Adminisztrátori megjegyzés</strong><p>Itt a telefonkönyv adminja tehet megjegyzést
                    a módosítás elvetésének, vagy megváltoztatásának okáról.</p>" ?>
