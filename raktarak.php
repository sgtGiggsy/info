...
else
{
    ...
    if($mindir) 
    {
        ?><button type="button" onclick="location.href='<?=$RootPath?>/raktarszerkeszt'">Új raktár</button><?php
    }
        ...
        <tbody><?php
            ...
            foreach($raktarak as $raktar)
            {
                ...
                ?><tr class='kattinthatotr-<?=($szamoz % 2 == 0) ? "2" : "1" ?>' data-href='./raktar/<?=$raktar['id']?>'>
                    ...
                    <td><?=($csoportir) ? "<a href='$RootPath/raktarszerkeszt/$raktarid'><img src='$RootPath/images/edit.png' alt='Raktár szerkesztése' title='Raktár szerkesztése'/></a>" : "" ?></td>
                </tr><?php
                $szamoz++;
            }
        ?></tbody>
